...
class Recorderi{
  ...
  Future<void> _sendRecordingToServer(File file) async {
    ...
    // Адрес нового Node.js обработчика распознавания
    final uri = Uri.parse('https://bladt.keep-pixel.ru/recognize');
    ...
    if(ds != "") {
      ...
      try {
        ...
        final response = await http.post(
          Uri.parse('https://bladt.keep-pixel.ru/music/${lisa['id']}/reaction'),
          headers: {
            "Authorization": "Bearer $ds",
            "Content-Type": "application/json",
          },
          body: jsonEncode({"type": int.tryParse(type) ?? 0}),
        );
